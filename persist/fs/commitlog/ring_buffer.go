...
func (rb *RingBuffer) Put(item readResponse) error {
...
// queue is disposed.
func (rb *RingBuffer) Offer(item readResponse) (bool, error) {
	return rb.put(item, true)
}

func (rb *RingBuffer) put(item readResponse, offer bool) (bool, error) {
	var n *node
...
func (rb *RingBuffer) Get() (readResponse, error) {
	return rb.Poll(0)
}
...
func (rb *RingBuffer) Poll(timeout time.Duration) (readResponse, error) {
	var (
		n     *node
...
L:
	for {
		if atomic.LoadUint64(&rb.disposed) == 1 {
			return readResponse{}, ErrDisposed
		}
		...
		if timeout > 0 && time.Since(start) >= timeout {
			return readResponse{}, ErrTimeout
		}
