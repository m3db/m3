...
function setup_docker {
  ...
  docker-compose -f ${COMPOSE_FILE} up -d --renew-anon-volumes m3comparator
  docker-compose -f ${COMPOSE_FILE} up -d --renew-anon-volumes prometheus
  docker-compose -f ${COMPOSE_FILE} up -d --renew-anon-volumes m3query

  CI=$1
	if [[ "$CI" != "true" ]]
  then
    echo "run grafana container"
    docker-compose -f ${COMPOSE_FILE} up -d --renew-anon-volumes grafana
  fi
}
