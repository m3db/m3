...
COMPOSE_FILE=$GOPATH/src/github.com/m3db/m3/scripts/docker-integration-tests/prometheus_maptags/docker-compose.yml
...
function prometheus_remote_write {
  ...
  out=$((docker run -it --rm --network $network           \
    ...
    -h 'M3-Map-Tags-By-JSON: {"tagMappers":[{"append":{"tag":"globaltag","value":"somevalue"}}]}' \
...
function prometheus_query_native {
  ...
  result=$(curl -s                                    \
    -H "M3-Metrics-Type: ${metrics_type}"             \
    -H "M3-Storage-Policy: ${metrics_storage_policy}" \
    "0.0.0.0:7201/api/v1/${endpoint}?query=${query}${params_prefixed}" | jq -r '.data.result[0].metric.globaltag')
  test "$result" = "somevalue"
  return $?
}
...
ATTEMPTS=50 TIMEOUT=2 MAX_TIMEOUT=4 \
    ...
    metrics_type="unaggregated" jq_path=".data.result[0].value[1]" expected_value="42.42" \
