...
function test_query_limits_global_applied {
  ...
  # Check that success and throttled header is returned.
  ATTEMPTS=20 TIMEOUT=1 MAX_TIMEOUT=1 retry_with_backoff  \
    '[[ $(curl -s -D headers.out 0.0.0.0:7201/api/v1/query?query=\\{query_global_limit_test!=\"\"\\} | jq -r ."status") = "success" ]] && [[ $(cat headers.out | grep M3-Throttled | wc -l | xargs) = "1" ]]'

  # Check that error when require no throttle header set and throttled header is returned.
  STATUS=$(curl -s -o /dev/null -w "%{http_code}" -H "M3-Limit-Require-NoThrottle: true" 0.0.0.0:7201/api/v1/query?query=\\{query_global_limit_test!=\"\"\\})
  test "$STATUS" = "400"
