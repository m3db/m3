...
function test_query_limits_applied {
  ...
  query_url="0.0.0.0:7201/api/v1/query_range?query=database_write_tagged_success&step=15&start=0&end=$(date +%s)"
  echo "Test query time range limit with coordinator defaults"
  ATTEMPTS=50 TIMEOUT=2 MAX_TIMEOUT=4 retry_with_backoff  \
    '[[ $(curl -s "$query_url" | jq -r ".data.result | length") -gt 0 ]]'

  echo "Test query time range limit with require-exhaustive headers false"
  ATTEMPTS=50 TIMEOUT=2 MAX_TIMEOUT=4 retry_with_backoff  \
    '[[ $(curl -s -H "M3-Limit-Max-Range: 4h" -H "M3-Limit-Require-Exhaustive: false" "$query_url" | jq -r ".data.result | length") -gt 0 ]]'
