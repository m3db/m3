...
curl -vvvsSf -X POST -H "Cluster-Name-Environment: override_test_env" localhost:7201/api/v1/services/m3aggregator/placement/init -d '{
    "num_shards": 64,
...
curl -vvvsSf -X POST -H "Cluster-Name-Environment: override_test_env" localhost:7201/api/v1/topic/init -d '{
    "numberOfShards": 64
}'
...
curl -vvvsSf -X POST -H "Cluster-Name-Environment: override_test_env" localhost:7201/api/v1/topic -d '{
  "consumerService": {
    "serviceId": {
      "name": "m3coordinator",
