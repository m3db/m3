...
execute() {
  ...
  install -d "${BINDIR}"
  for binexe in "gometalinter" "gocyclo" "nakedret" "misspell" "govet" "golint" "ineffassign" "goconst" "errcheck" "maligned" "unconvert" "dupl" "structcheck" "varcheck" "safesql" "deadcode" "lll" "goimports" "gotype" "gosimple" "megacheck" "staticcheck" "unused" "interfacer" "unparam" ; do
    if [ "$OS" = "windows" ]; then
      binexe="${binexe}.exe"
    fi
