...
for i in "${GOPATH}/src/${PROTO_SRC}"/*; do
	...
	if ls $i/*.proto > /dev/null 2>&1; then
		proto_files=$(ls $i/*.proto | sed -e "s@${GOPATH}@/go@g")
    ...
    protoc --gogofaster_out=${resolve_protos},plugins=grpc:/go/src \
      -I/go/src -I/go/src/github.com/m3db/m3/vendor ${proto_files}
