...
if check_native_protoc; then
    ...
    UID_FLAGS="-u $(id -u)"
    if [[ -n "$BUILDKITE" ]] || [[ -n "$FORCE_DOCKER_ROOT" ]]; then
        UID_FLAGS="-u root"
    fi
    ...
    # Check if we have dependency-aware ordering (for metrics module) or simple iteration (for other modules)
    proto_base_dir="${GOPATH}/src/${PROTO_SRC}"
    if [[ "$PROTO_SRC" == *"metrics/generated/proto"* ]]; then
        # Use dependency-aware ordering for metrics
        for dir in "${ordered_dirs[@]}"; do
            proto_dir="${proto_base_dir}/${dir}"
            if [ -d "$proto_dir" ] && ls $proto_dir/*.proto > /dev/null 2>&1; then
        ...
        # Use simple iteration for other modules (maintain backward compatibility)
        for i in "${proto_base_dir}"/*; do
            if ! [ -d $i ]; then
                continue
            fi
