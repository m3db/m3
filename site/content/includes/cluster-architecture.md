...
- **[Storage nodes](/docs/components/m3db)** (`m3dbnode`) are the workhorses of M3, they store data and serve reads and writes.
- **[Coordinator nodes](/docs/components/m3coordinator)** (`m3coordinator`) coordinate reads and writes across all nodes in the cluster. It's a lightweight process, and does not store any data. This role typically runs alongside a Prometheus instance, or is part of a collector agent such as statsD.
...
- **[Query nodes](/docs/components/m3query)** (`m3query`) are an alternative query option to using M3's built-in PromQL support.
