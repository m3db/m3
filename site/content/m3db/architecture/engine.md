...
M3DB takes a two-pronged approach to persistent storage that involves combining a [commit log](/docs/operational_guide/commitlogs) for disaster recovery with periodic flushing (writing [fileset files](/docs/operational_guide/storage) to disk) for efficient retrieval:
