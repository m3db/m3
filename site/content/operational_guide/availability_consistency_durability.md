...
This means that all writes must be acknowledged by a quorum of nodes in order to be considered successful, and that reads will attempt to achieve quorum, but will return the data from a single node if they are unable to achieve quorum. This ensures that reads are consistent under normal conditions, while degraded conditions will not cause reads to fail outright as long as at least a single node can satisfy the request.
