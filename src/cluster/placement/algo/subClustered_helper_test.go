...
func TestGoodCaseSubClusters(t *testing.T) {
	...
	i7 := placement.NewEmptyInstance("g", "r11", "z1", "endpoint", 1).SetSubClusterID(2)
	i8 := placement.NewEmptyInstance("h", "r22", "z1", "endpoint", 1).SetSubClusterID(2)
	i9 := placement.NewEmptyInstance("i", "r33", "z1", "endpoint", 1).SetSubClusterID(2)
	i11 := placement.NewEmptyInstance("j", "r11", "z1", "endpoint", 1).SetSubClusterID(2)
	i12 := placement.NewEmptyInstance("k", "r22", "z1", "endpoint", 1).SetSubClusterID(2)
	i13 := placement.NewEmptyInstance("l", "r33", "z1", "endpoint", 1).SetSubClusterID(2)
	...
	p, err = algo.AddInstances(p, []placement.Instance{i7, i8, i9, i11, i12, i13})
	require.NoError(t, err)
	require.NoError(t, placement.Validate(p))
	p, _, err = algo.MarkAllShardsAvailable(p)
	assert.NoError(t, err)
	...
	fmt.Println("============================================================================")
	p, err = algo.RemoveInstances(p, []string{"a", "b", "c", "d", "e", "f"})
	require.NoError(t, err)
	require.NoError(t, placement.Validate(p))
	p, _, err = algo.MarkAllShardsAvailable(p)
	assert.NoError(t, err)
