...
func benchmarkThrottling(users int, concurrencyPerUser int, totalAcquiresPerRoutine int) {
	...
	for user := 0; user < users; user++ {
		...
		for routine := 0; routine < concurrencyPerUser; routine++ {
			wg.Add(1)
			go func() {
				for req := 0; req < totalAcquiresPerRoutine; req++ {
