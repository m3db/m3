...
// WeightedThrottler controls fair access to limited resources.
type WeightedThrottler struct {
	sync.Mutex

	keyState map[string]*weightedKeyContext
...
type weightedKeyContext struct {
	// Requests for weight to be granted which are waiting.
	waiting []weightedRequest
...
func (t *WeightedThrottler) Acquire(key string, weight int) error {
...
func (t *WeightedThrottler) tryAcquire(key string, weight int) (chan struct{}, error) {
	t.Lock()
	defer t.Unlock()
	...
	if currentKey.currentWeight+weight <= maxWeightPerKey && t.globalCurrentWeight+weight <= t.globalMaxWeight {
		currentKey.currentWeight += weight
		t.globalCurrentWeight += weight
	} else {
		blockCh := make(chan struct{}, 0)
		currentKey.waiting = append(currentKey.waiting, weightedRequest{
			blockCh: blockCh,
			weight:  weight,
		})
...
func (t *WeightedThrottler) Release(key string, weight int) {
	t.Lock()
	defer t.Unlock()
