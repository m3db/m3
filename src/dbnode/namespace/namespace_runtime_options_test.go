...
func TestRuntimeOptionsManager_RegisterListener(t *testing.T) {
	...
	time.Sleep(10 * time.Millisecond) // Allow goroutine to receive update

	newFlush := 0.99
	err := manager.Update(NewRuntimeOptions().SetFlushIndexingPerCPUConcurrency(&newFlush))
	assert.NoError(t, err)

	time.Sleep(10 * time.Millisecond) // Wait for update propagation
...
func TestRuntimeOptionsManagerRegistry(t *testing.T) {
	...
	assert.Equal(t, manager1, manager2) // Ensure it's cached
	...
	// After Close(), re-creation of manager should be a new one
