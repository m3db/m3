...
At a high level, the binary format begins with a stream header and then can be thought of as a sequence of tuples in the form: `<per write header, compressed timestamp, compressed custom encoded fields, Protobuf marshaled fields>`
...
If the control bit is `1` (meaning this is not the end of the stream), then the next two bits should be interpreted as boolean control bits indicating there has been a time unit or schema change respectively.
...
The header ends immediately after combinations #1 and #2, but combinations #3, #4, and #5 will be followed by the encoder time unit change and/or schema change.
...
Note that only fields that support custom encoding are included in the schema. This is because the Protobuf encoding format will take care of schema changes for any fields that are not being custom encoded as long as all schema changes [are valid message updates according to the Protobuf specification](https://developers.google.com/protocol-buffers/docs/proto3#updating).
