...
func sumSeries(ctx *querycontext.Context, series multiplePathSpecs) (ts.SeriesList, error) {
...
// If the time series have different intervals, the coarsest interval will be used.
func diffSeries(ctx *querycontext.Context, series multiplePathSpecs) (ts.SeriesList, error) {
	...
	if numSeries > 1 {
		...
		for i := 1; i < len(series.Values); i++ {
			res, err := transform(
				...
				querycontext.MaintainNaNTransformer(func(v float64) float64 { return -v }),
			)
			if err != nil {
...
// If the time series have different intervals, the coarsest interval will be used.
func multiplySeries(ctx *querycontext.Context, series multiplePathSpecs) (ts.SeriesList, error) {
...
func averageSeries(ctx *querycontext.Context, series multiplePathSpecs) (ts.SeriesList, error) {
...
func minSeries(ctx *querycontext.Context, series multiplePathSpecs) (ts.SeriesList, error) {
...
func maxSeries(ctx *querycontext.Context, series multiplePathSpecs) (ts.SeriesList, error) {
...
func divideSeries(ctx *querycontext.Context, dividendSeriesList, divisorSeriesList singlePathSpec) (ts.SeriesList, error) {
	...
	for idx, dividendSeries := range dividendSeriesList.Values {
		normalized, minBegin, _, lcmMillisPerStep, err := querycontext.Normalize(ctx, ts.SeriesList{
...
func groupByNode(ctx *querycontext.Context, series singlePathSpec, node int, fname string) (ts.SeriesList, error) {
	...
	for _, s := range series.Values {
		...
		if n >= len(parts) || n < 0 {
			err := fmt.Errorf("could not group %s by node %d; not enough parts", s.Name(), node)
			return ts.SeriesList{}, err
		}
	...
	if !fexists {
		return ts.SeriesList{}, fmt.Errorf("invalid func %s", fname)
	}
...
func combineSeries(ctx *querycontext.Context,
	...
	normalized, start, end, millisPerStep, err := querycontext.Normalize(ctx, ts.SeriesList(series))
	if err != nil {
		err := fmt.Errorf("combine series error: %v", err)
		return ts.SeriesList{}, err
	}
...
func countSeries(ctx *querycontext.Context, seriesList multiplePathSpecs) (ts.SeriesList, error) {
	count, err := querycontext.Count(ctx, ts.SeriesList(seriesList), func(series ts.SeriesList) string {
