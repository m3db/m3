...
func sortByName(_ *querycontext.Context, series singlePathSpec) (ts.SeriesList, error) {
...
func sortByTotal(ctx *querycontext.Context, series singlePathSpec) (ts.SeriesList, error) {
...
func sortByMaxima(ctx *querycontext.Context, series singlePathSpec) (ts.SeriesList, error) {
...
func maximumAbove(ctx *querycontext.Context, series singlePathSpec, n float64) (ts.SeriesList, error) {
...
func minimumAbove(ctx *querycontext.Context, series singlePathSpec, n float64) (ts.SeriesList, error) {
...
func averageAbove(ctx *querycontext.Context, series singlePathSpec, n float64) (ts.SeriesList, error) {
...
func currentAbove(ctx *querycontext.Context, series singlePathSpec, n float64) (ts.SeriesList, error) {
...
func currentBelow(ctx *querycontext.Context, series singlePathSpec, n float64) (ts.SeriesList, error) {
	sr := ts.SeriesReducerLast.Reducer()
	return belowByFunction(ctx, series, sr, n)
}

// constantLine takes value and creates a constant line at value.
func constantLine(ctx *querycontext.Context, value float64) (ts.SeriesList, error) {
	newSeries, err := querycontext.ConstantLine(ctx, value)
	if err != nil {
...
func identity(ctx *querycontext.Context, name string) (ts.SeriesList, error) {
	return querycontext.Identity(ctx, name)
}

// limit takes one metric or a wildcard seriesList followed by an integer N, and draws
// the first N metrics.
func limit(_ *querycontext.Context, series singlePathSpec, n int) (ts.SeriesList, error) {
	if n < 0 {
		return ts.SeriesList{}, fmt.Errorf("invalid limit parameter n: %d", n)
	}
...
func timeShift(
	...
	shift, err := querycontext.ParseInterval(timeShiftS)
	if err != nil {
		return nil, fmt.Errorf("invalid timeShift parameter %s: %v", timeShiftS, err)
	}

	contextShiftingFn := func(c *querycontext.Context) *querycontext.Context {
		opts := querycontext.NewChildContextOptions()
...
func absolute(ctx *querycontext.Context, input singlePathSpec) (ts.SeriesList, error) {
...
func scale(ctx *querycontext.Context, input singlePathSpec, scale float64) (ts.SeriesList, error) {
	return transform(
		ctx,
		input,
		func(fname string) string {
			newName := fmt.Sprintf("%s,"+querycontext.FloatingPointFormat, fname, scale)
			return fmt.Sprintf(wrappingFmt, "scale", newName)
		},
		querycontext.Scale(scale),
	)
}
...
func offset(ctx *querycontext.Context, input singlePathSpec, factor float64) (ts.SeriesList, error) {
	return transform(
		ctx,
		input,
		func(fname string) string {
			newName := fmt.Sprintf("%s,"+querycontext.FloatingPointFormat, fname, factor)
			return fmt.Sprintf(wrappingFmt, "offset", newName)
		},
		querycontext.Offset(factor),
	)
}

// transform converts values in a timeseries according to the valueTransformer.
func transform(ctx *querycontext.Context, input singlePathSpec,
	fname func(inputName string) string, fn querycontext.TransformFunc) (ts.SeriesList, error) {

	t := querycontext.NewStatelessTransformer(fn)
	return querycontext.Transform(ctx, ts.SeriesList(input), t, func(in *ts.Series) string {
...
func perSecond(ctx *querycontext.Context, input singlePathSpec, _ float64) (ts.SeriesList, error) {
	// TODO(jayp):  we are ignoring maxValue; we may need to implement it
	return querycontext.PerSecond(ctx, ts.SeriesList(input), func(series *ts.Series) string {
		return fmt.Sprintf("perSecond(%s)", series.Name())
	})
}

// transformNull transforms all nulls (NaNs) in a time series to a defaultValue.
func transformNull(ctx *querycontext.Context, input singlePathSpec, defaultValue float64) (ts.SeriesList, error) {
	return transform(
		ctx,
		input,
		func(fname string) string {
			newName := fmt.Sprintf("%s,"+querycontext.FloatingPointFormat, fname, defaultValue)
			return fmt.Sprintf(wrappingFmt, "transformNull", newName)
		},
		querycontext.TransformNull(defaultValue),
	)
}
...
func isNonNull(ctx *querycontext.Context, input singlePathSpec) (ts.SeriesList, error) {
	return transform(ctx,
		input,
		func(fname string) string { return fmt.Sprintf(wrappingFmt, "isNonNull", fname) },
		querycontext.IsNonNull())
}
...
func keepLastValue(ctx *querycontext.Context, input singlePathSpec, limit int) (ts.SeriesList, error) {
...
func sustainedCompare(ctx *querycontext.Context, input singlePathSpec, threshold float64, intervalString string,
	comparisonFunction comparator, zeroValue float64, funcName string) (ts.SeriesList, error) {
	output := make([]*ts.Series, 0, len(input.Values))
	interval, err := querycontext.ParseInterval(intervalString)
	if err != nil {
		return ts.SeriesList{}, err
	}
...
func sustainedAbove(ctx *querycontext.Context, input singlePathSpec, threshold float64, intervalString string) (ts.SeriesList, error) {
...
func sustainedBelow(ctx *querycontext.Context, input singlePathSpec, threshold float64, intervalString string) (ts.SeriesList, error) {
...
func removeBelowValue(ctx *querycontext.Context, input singlePathSpec, n float64) (ts.SeriesList, error) {
	return transform(ctx, input,
		func(inputName string) string {
			return fmt.Sprintf("removeBelowValue(%s, "+querycontext.FloatingPointFormat+")", inputName, n)
		},
		querycontext.Filter(func(v float64) bool { return v >= n }))
}
...
func removeAboveValue(ctx *querycontext.Context, input singlePathSpec, n float64) (ts.SeriesList, error) {
	return transform(ctx, input,
		func(inputName string) string {
			return fmt.Sprintf("removeAboveValue(%s, "+querycontext.FloatingPointFormat+")", inputName, n)
		},
		querycontext.Filter(func(v float64) bool { return v <= n }))
}

// removeEmptySeries returns only the time-series with non-empty data
func removeEmptySeries(ctx *querycontext.Context, input singlePathSpec) (ts.SeriesList, error) {
	return querycontext.RemoveEmpty(ctx, ts.SeriesList(input))
}
...
func highestSum(_ *querycontext.Context, input singlePathSpec, n int) (ts.SeriesList, error) {
...
func highestMax(_ *querycontext.Context, input singlePathSpec, n int) (ts.SeriesList, error) {
	sr := ts.SeriesReducerMax.Reducer()
	return takeByFunction(input, n, sr, ts.Descending)
}
...
func highestCurrent(_ *querycontext.Context, input singlePathSpec, n int) (ts.SeriesList, error) {
	sr := ts.SeriesReducerLast.Reducer()
	return takeByFunction(input, n, sr, ts.Descending)
}
...
func highestAverage(_ *querycontext.Context, input singlePathSpec, n int) (ts.SeriesList, error) {
	sr := ts.SeriesReducerAvg.Reducer()
	return takeByFunction(input, n, sr, ts.Descending)
}
...
func fallbackSeries(_ *querycontext.Context, input singlePathSpec, fallback singlePathSpec) (ts.SeriesList, error) {
...
func mostDeviant(_ *querycontext.Context, input singlePathSpec, n int) (ts.SeriesList, error) {
...
func lowestAverage(_ *querycontext.Context, input singlePathSpec, n int) (ts.SeriesList, error) {
	sr := ts.SeriesReducerAvg.Reducer()
...
func lowestCurrent(_ *querycontext.Context, input singlePathSpec, n int) (ts.SeriesList, error) {
	sr := ts.SeriesReducerLast.Reducer()
	return takeByFunction(input, n, sr, ts.Ascending)
}
...
func movingAverage(ctx *querycontext.Context, input singlePathSpec, windowSizeValue genericInterface) (*binaryContextShifter, error) {
	if len(input.Values) == 0 {
		return nil, nil
	}
	...
	case string:
		interval, err := querycontext.ParseInterval(windowSizeValue)
		if err != nil {
			return nil, err
		}
		if interval <= 0 {
...
func asPercent(ctx *querycontext.Context, input singlePathSpec, total genericInterface) (ts.SeriesList, error) {
	if len(input.Values) == 0 {
		return ts.SeriesList(input), nil
	}
	...
	case ts.SeriesList, singlePathSpec:
		...
		if total.Len() == 0 {
			...
			if total.Len() != 1 {
				err := errors.New("total must be a single series")
				return ts.SeriesList{}, err
			}
	...
	case float64:
		...
		totalText = fmt.Sprintf(querycontext.FloatingPointFormat, totalArg)
	default:
		err := errors.New("total is neither an int nor a series")
		return ts.SeriesList{}, err
	}

	result, _, _, _, err := querycontext.Normalize(ctx, ts.SeriesList{
		Values: toNormalize,
	})
	if err != nil {
		return ts.SeriesList{}, err
...
// expression.
func exclude(_ *querycontext.Context, input singlePathSpec, pattern string) (ts.SeriesList, error) {
...
// logarithmic format.
func logarithm(ctx *querycontext.Context, input singlePathSpec, base int) (ts.SeriesList, error) {
	if base <= 0 {
		err := fmt.Errorf("invalid log base %d", base)
		return ts.SeriesList{}, err
	}
...
func group(_ *querycontext.Context, input multiplePathSpecs) (ts.SeriesList, error) {
	return ts.SeriesList(input), nil
}

func derivativeTemplate(ctx *querycontext.Context, input singlePathSpec, nameTemplate string,
...
func integral(ctx *querycontext.Context, input singlePathSpec) (ts.SeriesList, error) {
	results := make([]*ts.Series, 0, len(input.Values))
	for _, series := range input.Values {
		if series.AllNaN() {
...
// points.
func derivative(ctx *querycontext.Context, input singlePathSpec) (ts.SeriesList, error) {
...
func nonNegativeDerivative(ctx *querycontext.Context, input singlePathSpec, maxValue float64) (ts.SeriesList, error) {
...
func nPercentile(ctx *querycontext.Context, seriesList singlePathSpec, percentile float64) (ts.SeriesList, error) {
	return querycontext.NPercentile(ctx, ts.SeriesList(seriesList), percentile, func(name string, percentile float64) string {
		return fmt.Sprintf("nPercentile(%s, "+querycontext.FloatingPointFormat+")", name, percentile)
	})
}

func percentileOfSeries(ctx *querycontext.Context, seriesList singlePathSpec, percentile float64, interpolateValue genericInterface) (ts.SeriesList, error) {
	...
	if len(seriesList.Values) == 0 {
		err := fmt.Errorf("series list cannot be empty")
		return ts.SeriesList{}, err
	}

	normalize, _, _, _, err := querycontext.Normalize(ctx, ts.SeriesList(seriesList))
	if err != nil {
		return ts.SeriesList{}, err
	}
	...
	for _, series := range seriesList.Values[1:] {
		if step != series.MillisPerStep() {
			err := fmt.Errorf("different step sizes in input series not supported")
			return ts.SeriesList{}, err
		}
	}
	...
	for i := 0; i < min; i++ {
		...
		percentiles[i] = querycontext.GetPercentile(row, percentile, interpolate)
	}
	...
	name := fmt.Sprintf("percentileOfSeries(%s,"+querycontext.FloatingPointFormat+")",
...
func hitcount(ctx *querycontext.Context, seriesList singlePathSpec, intervalString string) (ts.SeriesList, error) {
	interval, err := querycontext.ParseInterval(intervalString)
	if err != nil {
		return ts.SeriesList{}, err
	}
	...
	if intervalInSeconds <= 0 {
		return ts.SeriesList{}, querycontext.ErrInvalidIntervalFormat
	}
...
// are passed).
func substr(_ *querycontext.Context, seriesList singlePathSpec, start, stop int) (ts.SeriesList, error) {
	...
	for i, series := range seriesList.Values {
		...
		if start < 0 || start >= numParts || (stop != 0 && stop < start) {
			err := fmt.Errorf("invalid substr params, start=%d, stop=%d", start, stop)
			return ts.SeriesList{}, err
		}
		var newName string
		if stop == 0 {
...
func trimBootstrap(ctx *querycontext.Context, bootstrap, original *ts.Series) *ts.Series {
...
func holtWintersForecast(ctx *querycontext.Context, seriesList singlePathSpec) (ts.SeriesList, error) {
	return holtWintersForecastInternal(ctx, seriesList, secondsPerWeek*time.Second)
}

// nolint: unparam
func holtWintersForecastInternal(ctx *querycontext.Context, seriesList singlePathSpec, d time.Duration) (ts.SeriesList, error) {
	results := make([]*ts.Series, len(seriesList.Values))
	bootstrapList, err := querycontext.FetchWithBootstrap(ctx, ts.SeriesList(seriesList), d)
	if err != nil {
		return ts.SeriesList{}, err
	}
	for i, bootstrap := range bootstrapList.Values {
...
func holtWintersConfidenceBands(ctx *querycontext.Context, seriesList singlePathSpec, delta float64) (ts.SeriesList, error) {
	return holtWintersConfidenceBandsInternal(ctx, seriesList, delta, secondsPerWeek*time.Second)
}

// nolint: unparam
func holtWintersConfidenceBandsInternal(ctx *querycontext.Context, seriesList singlePathSpec, delta float64, d time.Duration) (ts.SeriesList, error) {
...
func holtWintersAberration(ctx *querycontext.Context, seriesList singlePathSpec, delta float64) (ts.SeriesList, error) {
...
func holtWintersAnalysis(ctx *querycontext.Context, series *ts.Series) *holtWintersAnalysisResult {
...
func squareRoot(ctx *querycontext.Context, seriesList singlePathSpec) (ts.SeriesList, error) {
	return transform(
		ctx,
		seriesList,
...
func stdev(ctx *querycontext.Context, seriesList singlePathSpec, points int, windowTolerance float64) (ts.SeriesList, error) {
	return querycontext.Stdev(ctx, ts.SeriesList(seriesList), points, windowTolerance, func(series *ts.Series, points int) string {
		return fmt.Sprintf("stddev(%s,%d)", series.Name(), points)
	})
}

// rangeOfSeries distills down a set of inputs into the range of the series.
func rangeOfSeries(ctx *querycontext.Context, seriesList singlePathSpec) (ts.SeriesList, error) {
	series, err := querycontext.Range(ctx, ts.SeriesList(seriesList), func(series ts.SeriesList) string {
...
// of None.
func removeAbovePercentile(ctx *querycontext.Context, seriesList singlePathSpec, percentile float64) (ts.SeriesList, error) {
	return querycontext.RemoveByPercentile(ctx,
		ts.SeriesList(seriesList),
		percentile,
		func(name string, percentile float64) string {
			return fmt.Sprintf("removeAbovePercentile(%s, "+querycontext.FloatingPointFormat+")", name, percentile)
		},
		querycontext.GreaterThan)
}
...
func removeBelowPercentile(ctx *querycontext.Context, seriesList singlePathSpec, percentile float64) (ts.SeriesList, error) {
	return querycontext.RemoveByPercentile(
		ctx,
		ts.SeriesList(seriesList),
		percentile,
		func(name string, percentile float64) string {
			return fmt.Sprintf("removeBelowPercentile(%s, "+querycontext.FloatingPointFormat+")", name, percentile)
		},
		querycontext.LessThan)
}

// randomWalkFunction returns a random walk starting at 0.
// Note: step has a unit of seconds.
func randomWalkFunction(ctx *querycontext.Context, name string, step int) (ts.SeriesList, error) {
	if step <= 0 {
		return ts.SeriesList{}, fmt.Errorf("non-positive step size %d", step)
	}
	if !ctx.StartTime.Before(ctx.EndTime) {
		return ts.SeriesList{}, fmt.Errorf("startTime %v is no earlier than endTime %v", ctx.StartTime, ctx.EndTime)
	}
...
func aggregateLine(ctx *querycontext.Context, seriesList singlePathSpec, f string) (ts.SeriesList, error) {
	if len(seriesList.Values) == 0 {
		return ts.SeriesList{}, querycontext.ErrEmptySeriesList
	}

	sa := ts.SeriesReducerApproach(f)
	r, ok := sa.SafeReducer()
	if !ok {
		return ts.SeriesList{}, fmt.Errorf("invalid function %s", f)
	}

	value := r(seriesList.Values[0])
	name := fmt.Sprintf("aggregateLine(%s,"+querycontext.FloatingPointFormat+")",
	...
	return ts.SeriesList{Values: []*ts.Series{renamed}}, nil
}

// changed takes one metric or a wildcard seriesList.
// Output 1 when the value changed, 0 when null or the same.
func changed(ctx *querycontext.Context, seriesList singlePathSpec) (ts.SeriesList, error) {
	return querycontext.Changed(ctx, ts.SeriesList(seriesList), func(series *ts.Series) string {
...
func movingMedian(ctx *querycontext.Context, _ singlePathSpec, windowSize string) (*binaryContextShifter, error) {
	interval, err := querycontext.ParseInterval(windowSize)
	if err != nil {
		return nil, err
	}
	if interval <= 0 {
		return nil, querycontext.ErrInvalidIntervalFormat
	}
	...
	transformerFn := func(bootstrapped, original ts.SeriesList) (ts.SeriesList, error) {
		...
		for i, bootstrap := range bootstrapList.Values {
			...
			if windowPoints <= 0 {
				err := fmt.Errorf("non positive window points, windowSize=%s, stepSize=%d",
			...
			for i := 0; i < numSteps; i++ {
				...
				nans := querycontext.SafeSort(window)
...
func legendValue(_ *querycontext.Context, seriesList singlePathSpec, valueType string) (ts.SeriesList, error) {
	sa := ts.SeriesReducerApproach(valueType)
	reducer, ok := sa.SafeReducer()
	if !ok {
		return ts.SeriesList{}, fmt.Errorf("invalid function %s", valueType)
	}

	results := make([]*ts.Series, 0, len(seriesList.Values))
	for _, series := range seriesList.Values {
		value := reducer(series)
		newName := fmt.Sprintf("%s (%s: "+querycontext.FloatingPointFormat+")", series.Name(), valueType, value)
...
func cactiStyle(_ *querycontext.Context, seriesList singlePathSpec) (ts.SeriesList, error) {
	if len(seriesList.Values) == 0 {
		return ts.SeriesList{}, querycontext.ErrEmptySeriesList
	}
...
func consolidateBy(_ *querycontext.Context, seriesList singlePathSpec, consolidationApproach string) (ts.SeriesList, error) {
	ca := ts.ConsolidationApproach(consolidationApproach)
	cf, ok := ca.SafeFunc()
	if !ok {
		err := fmt.Errorf("invalid consolidation approach %s", consolidationApproach)
		return ts.SeriesList{}, err
	}
...
func offsetToZero(ctx *querycontext.Context, seriesList singlePathSpec) (ts.SeriesList, error) {
	results := make([]*ts.Series, len(seriesList.Values))
...
func timeFunction(ctx *querycontext.Context, name string, step int) (ts.SeriesList, error) {
	if step <= 0 {
		return ts.SeriesList{}, fmt.Errorf("step must be a positive int but instead is %d", step)
	}
...
func dashed(_ *querycontext.Context, seriesList singlePathSpec, dashLength float64) (ts.SeriesList, error) {
	if dashLength <= 0 {
		return ts.SeriesList{}, fmt.Errorf("expected a positive dashLength but got %f", dashLength)
	}

	results := make([]*ts.Series, len(seriesList.Values))
	for idx, s := range seriesList.Values {
		name := fmt.Sprintf("dashed(%s, "+querycontext.FloatingPointFormat+")", s.Name(), dashLength)
		renamed := s.RenamedTo(name)
		if renamed.GraphOptions == nil {
			renamed.GraphOptions = make(map[string]string)
		}
		renamed.GraphOptions["dashed"] = fmt.Sprintf(querycontext.FloatingPointFormat, dashLength)
		results[idx] = renamed
	}
...
func threshold(ctx *querycontext.Context, value float64, label string, color string) (ts.SeriesList, error) {
	seriesList, err := constantLine(ctx, value)
	if err != nil {
		err := fmt.Errorf("error applying threshold function, error=%v", err)
		return ts.SeriesList{}, err
	}
