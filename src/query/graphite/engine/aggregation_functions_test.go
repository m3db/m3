...
func newConsolidationTestSeries() (*querycontext.Context, []*ts.Series) {
	ctx := querycontext.NewContext(querycontext.ContextOptions{Start: consolidationStartTime, End: consolidationEndTime})
...
func testAggregatedSeries(
	t *testing.T,
	f func(ctx *querycontext.Context, series multiplePathSpecs) (ts.SeriesList, error),
...
func TestSumSeries(t *testing.T) {
	testAggregatedSeries(t, func(ctx *querycontext.Context, series multiplePathSpecs) (ts.SeriesList, error) {
...
func TestDivideSeries(t *testing.T) {
	...
	querycontext.CompareOutputsAndExpected(t, 10000, consolidationStartTime,
		[]xtest.TestSeries{expected[0]}, []*ts.Series{series.Values[0]})
	querycontext.CompareOutputsAndExpected(t, 10000, consolidationStartTime.Add(-30*time.Second),
		[]xtest.TestSeries{expected[1]}, []*ts.Series{series.Values[1]})
...
func TestAverageSeriesWithWildcards(t *testing.T) {
	...
	input := []xtest.TestSeries{
		xtest.TestSeries{"web.host-1.avg-response.value", []float64{70.0, 20.0, 30.0, 40.0, 50.0}},
		xtest.TestSeries{"web.host-2.avg-response.value", []float64{20.0, 30.0, 40.0, 50.0, 60.0}},
		xtest.TestSeries{"web.host-3.avg-response.value", []float64{30.0, 40.0, 80.0, 60.0, 70.0}},
		xtest.TestSeries{"web.host-4.num-requests.value", []float64{10.0, 10.0, 15.0, 10.0, 15.0}},
	}
	expected := []xtest.TestSeries{
		xtest.TestSeries{"web.avg-response", []float64{40.0, 30.0, 50.0, 50.0, 60.0}},
		xtest.TestSeries{"web.num-requests", []float64{10.0, 10.0, 15.0, 10.0, 15.0}},
	}
	...
	querycontext.CompareOutputsAndExpected(t, step, start, expected, output.Values)
}
...
func TestSumSeriesWithWildcards(t *testing.T) {
	var (
		...
		ctx      = querycontext.NewContext(querycontext.ContextOptions{Start: start, End: end})
		inputs   = []*ts.Series{
...
func TestWeightedAverage(t *testing.T) {
	...
	means := []xtest.TestSeries{
		xtest.TestSeries{"web.host-1.avg-response.mean", []float64{70.0, 20.0, 30.0, 0.0, 50.0}},
		xtest.TestSeries{"web.host-2.avg-response.mean", []float64{20.0, 30.0, 40.0, 50.0, 60.0}},
		xtest.TestSeries{"web.host-3.avg-response.mean", []float64{20.0, 30.0, 40.0, 50.0, 60.0}}, // no match
	}
	counts := []xtest.TestSeries{
		xtest.TestSeries{"web.host-1.avg-response.count", []float64{1, 2, 3, 4, 5}},
		xtest.TestSeries{"web.host-2.avg-response.count", []float64{10, 20, 30, 40, 50}},
		xtest.TestSeries{"web.host-4.avg-response.count", []float64{10, 20, 30, 40, 50}}, // no match
	}
	expected := []xtest.TestSeries{
		xtest.TestSeries{"weightedAverage", []float64{24.5454, 29.0909, 39.0909, 45.4545, 59.0909}},
	}
	...
	output, err = weightedAverage(ctx, singlePathSpec(values), singlePathSpec(weights), 1)
	require.NoError(t, err)
	querycontext.CompareOutputsAndExpected(t, step, start,
		[]xtest.TestSeries{{"weightedAverage", means[0].Data}}, output.Values)
...
func TestCountSeries(t *testing.T) {
	...
	expected := testing.TestSeries{
		Name: "countSeries(a,b,c,d)",
		Data: []float64{4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4},
	}
	require.Nil(t, err)
	querycontext.CompareOutputsAndExpected(t, input[1].MillisPerStep(), input[1].StartTime(),
		[]xtest.TestSeries{expected}, results.Values)
}
